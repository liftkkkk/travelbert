...
class BertForEntityTyping(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        feature_output = hidden_states[:, 0]
        feature_output = self.dropout(feature_output)
        logits = self.typing(feature_output)
...
class TypingProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        examples = []
        for (i, line) in enumerate(tqdm(lines, desc="Examples")):
            guid = i
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, threshold):
    ...
    features = []
    for (ex_index, example) in enumerate(tqdm(examples, desc="Features")):
        ex_text_a = example.text_a[0]
        h = example.text_a[1][0]
        # ex_text_a = ex_text_a[:h[1]] + "。 " + ex_text_a[h[1]:h[2]] + " 。" + ex_text_a[h[2]:]
        tokens_a = tokenizer.tokenize(ex_text_a[:h[1]]) + ["[unused1]"] + tokenizer.tokenize(ex_text_a[h[1]:h[2]]) \
                    + ["[unused1]"] + tokenizer.tokenize(ex_text_a[h[2]:])
        begin, end = h[1:3]
...
def main():
    ...
    model = BertForEntityTyping.from_pretrained(args.bert_model, state_dict=state_dict,
    ...
    if args.do_train:
        train_feature_cache = "logs/train_features.pt"
        if os.path.exists(train_feature_cache):
            logger.info(f"Loading train features from {train_feature_cache}")
            train_features = torch.load(train_feature_cache)
        else:
            train_features = convert_examples_to_features(
                train_examples, label_list, args.max_seq_length, tokenizer, args.threshold)
            torch.save(train_features, train_feature_cache)
            logger.info(f"Saved train features to {train_feature_cache}")
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss = model(input_ids, segment_ids, input_mask, labels.half())
