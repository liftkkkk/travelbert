...
def log_loss(step_record, loss_record, model_name):
    ...
    plt.grid(True)
    plt.savefig(os.path.join("img", model_name+'-loss_curve.png'))
    plt.close()
...
def train(args, model, train_dataset):
    ...
    for i in range(args.max_epoch):
        for step, batch in enumerate(train_dataloader):
            if args.model == "CP" or args.model == "CLF" or args.model == "CAE":
            ...
            if step % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [0, -1] and global_step % 500 == 0:
                    log_loss(step_record, loss_record, args.model)
                
                if args.local_rank in [0, -1]:
                    sys.stdout.write("step: %d, shcedule: %.3f, mlm_loss: %.6f, r_loss: %.6f\r" % (global_step, global_step/step_tot, m_loss, r_loss))
                    sys.stdout.flush()
        if args.model == "CP" or args.model == "CAE":
            print("Sampling...")
...
if __name__ == "__main__":
    ...
    if args.model == "CP":
        ...
        model = CLF(args).to(args.device)
        train_dataset = CLFDataset("data/CLF", args)
    elif args.model == "CAE":
        model = CAE(args).to(args.device)
        train_dataset = CAEDataset("data/CAE", args)
    elif args.model == "BERT":
        model = BaikePretrain(args).to(args.device)
        train_dataset = BaikeDataset("data/morebaidubaike", args)
    else:
